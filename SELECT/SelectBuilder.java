...
public class SelectBuilder {
        ...
        private void navigate( Value node, String[] tokens, String path ) {
                ...
                if( "*[*]".equals( currToken ) ) {
                        ...
                        // Descendant pattern: ..field or ..field[*]
                        final String remainder = currToken.substring( 2 );
                        if( remainder.endsWith( "[*]" ) ) {
                                final String fieldName = remainder.replace( "[*]", "" );
                                searchDescendantArray( node, fieldName, path, otherTokens );
                        } else {
                                final String fieldName = remainder;
        ...
        private void searchDescendantArray( Value node, String fieldName, String path, String[] otherTokens ) {
                if( node.hasChildren( fieldName ) ) {
                        final ValueVector fieldArray = node.getChildren( fieldName );
                        final String fieldPath = path( path, fieldName );
                        array( fieldArray, fieldPath, otherTokens );
                ...
                node.children().forEach( (childName, childArray) -> {
                        if( !childArray.isEmpty() ) {
                                final Value firstChild = childArray.first();
                                final String childPath = path( path, childName );
                                searchDescendantArray( firstChild, fieldName, childPath, otherTokens );
                        }
                });
        }
